...
export default function DeckSearchPanel({
  ...
  return (
    <div className="bg-navy-light rounded-xl border border-cyan-dim p-4 flex flex-col min-h-[640px]">
      <h2 className="text-xl font-semibold text-cream mb-3">Card Search</h2>
      ...
      <div className="space-y-3 overflow-y-auto pr-1">
        {visibleResults.map(card => {
          ...
          return (
            <div key={`${card.id}-${card.collector_number}`} className="bg-navy rounded-lg border border-cyan-dim p-3">
              <div className="flex gap-3">
                <div className="w-16 shrink-0">
                  {imageUrl ? (
                    <img src={imageUrl} alt={card.name} className="w-16 rounded-md" />
                  ) : (
